...
I = double( imread('./1效果好的图/12.jpeg'));
...
Imin = DarkImage_ave_local-max(3.*DarkImage_std_local,0.001); % 计算暗通道

Idark=max(Imin,DarkImage); % 阈值处理
%%%%%%%%%%%%%%%%%%%%%%%%%
...
t = 1-0.25*(1-1.15*(Idark./A1)); % 计算传输函数
t0=0.1;  % 传输函数阈值
...
figure(1),imshow(OriImage),title('昏暗图像');
%figure(2),imshow(Idark),title('亮通道');
%figure(3),imshow(t),title('传输函数');
figure(4),imshow(Recover),title('亮通道先验还原的图像');
saveas(4,'./1效果好的图/recover/4.jpg')
